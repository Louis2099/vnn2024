function solve(problems::DemandShiftingProblem, max_branches=50, branch_management=false, incremental_computation=false, samples=nothing)
    ...
    problem = Problem(problems.network, problems.input, problems.outputs[1])
    ...
    timed_result = @timed check_all_leaves_demand_shifting(solver, problem, branches, incremental_computation)
    ...
    for (i, output) in enumerate(problems.outputs)
