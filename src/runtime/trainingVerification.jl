...
function calc_area(branches, result_dict)
    ...
    display(p)
    println("hold area ratio:", hold_area / area(inputset))
end

function solve(problems::TrainingProblem, max_branches=50, branch_management=false, perturbation_tolerence=0.0)
    ...
    for (i, nnet) in enumerate(problems.networks)
        ...
        if branch_management && cnts[2] > last_unk_cnt[2]
            result, branches = init_split(solver, problem, max_branches, splits_order)
            timed_result2 = @timed check_all_leaves(solver, problem, branches)
            result, result_dict, cnts = timed_result2.value
            last_unk_cnt = cnts

            timed_result.time = timed_result.time + timed_result2.time
            timed_result.value = timed_result2.value
        end
        println("cnts: ",cnts)
